...
class Ticket_page:
    def __init__(self, root2):
        ...
        self.conn = mysql.connector.connect(host="localhost", user="root", password="@Ashish0110", database="zoo")
        ...
        self.btn_back = ttk.Button(DataFrameRight, text="Back", command=self.back,style="TButton")
        self.btn_back.place(x=270,y=250)
    ...
    def select_date(self):
        ...
        self.calendar = Calendar(self.calendar_window, selectmode="day", year=current_date.year, 
                                  ...
                                  date_format='y-mm-dd HH:MM:SS',mindate=current_date)
    ...
    def print_ticket(self):
        self.ticket_no = random.randint(1, 1000)
    ...
    def print_ticket_pdf(self):
        # Load ticket template image
        ticket_template = Image.open("ticket1.jpg")
        draw = ImageDraw.Draw(ticket_template)

        # Set font and size
        font = ImageFont.truetype("arial.ttf", 16)

        # Define left margin, top margin, and line spacing
        left_margin = 200  # Adjust this value as needed
        top_margin = 170    # Adjust this value as needed
        line_spacing = 5   # Adjust this value to reduce spacing between lines

        # Draw ticket content with adjusted margins and line spacing
        draw.text((left_margin, top_margin), "Ticket ID:  " + str(self.ticket_no), fill="black", font=font)
        draw.text((left_margin, top_margin + font.getsize("Ticket ID:  ")[1] + line_spacing),
                "Date of Visit:  " + self.date_str, fill="black", font=font)
        draw.text((left_margin, top_margin + 2 * (font.getsize("Ticket ID:  ")[1] + line_spacing)),
                "Ticket Price: Rs.  " + str(self.ticket_price)+"/-", fill="black", font=font)
        draw.text((left_margin, top_margin + 3 * (font.getsize("Ticket ID:  ")[1] + line_spacing)),
                "No. of Tickets:  " + str(self.num_tickets), fill="black", font=font)

        # Save modified image as PDF
        ticket_template.save("ticket.pdf")

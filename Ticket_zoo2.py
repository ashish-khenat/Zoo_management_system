...
class Login_Page:
    def __init__(self, root1):
        ...
        lbltitle=Label(self.root1,text="WELCOME  TO  ZOO  ",bd=5,relief=FLAT,
                        bg='red',fg="white",font=("times new roman",50,"bold"),padx=2,pady=4)
    ...
    def process_signup(self, username, password, confirm_password, signup_window):
        ...
        if not username or not password:
        ...
        elif not re.match(r'^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()_+])(?!.*\s).{8,}$', password):
            ...
            conn = mysql.connector.connect(host="localhost", user="root", password="@Ashish0110", database="zoo")
